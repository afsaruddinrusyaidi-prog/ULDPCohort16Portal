...
**Game Master (Zaim).** The Game Master is the single approver. Nightly at 10 PM he opens the **Staff Console** (the staff URL, unlocked with the `STAFF_KEY`), works the **Pending** tab, and checks each claim's evidence against the `Catalog` entry for that activity. He can correct the House, category, points or member inline, then clicks **Approve** or **Reject** (with a one-line reason). Approved claims are aggregated into `House_Standings` and `MVP` automatically and the public board refreshes within ~1 minute. He then spot-checks the public leaderboard re-rendered correctly and posts the nightly screenshot to the cohort WhatsApp group. Nothing appears publicly without his approval. (The console writes back to the `Submissions` tab — he never edits rows by hand.)
...
| Time | Step |
|---|---|
| 10:00 PM | Open the Staff Console, unlock, go to **Pending** |
| 10:02 | Review each claim vs `Catalog` (evidence, caps, duplicates, house membership) → Approve/Reject |
| 10:12 | Check the **Standings** tab in the console; sanity-check vs yesterday |
